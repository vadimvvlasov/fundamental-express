...
def find_row(df, keywords, default_val=0.0):
    for kw in keywords:
        for idx in df.index:
            if kw.lower() == idx.lower():  # Exact match first
                return df.loc[idx]
        for idx in df.index:
            if kw.lower() in idx.lower():  # Partial match
                return df.loc[idx]
...
def get_company_data(ticker):
    ...
    if YFINANCE_AVAILABLE:
        try:
            ...
            if not financials.empty and not balance.empty and not cashflow.empty:
                online_success = True

                # Fetch shares and price
                price = info.get("currentPrice") or info.get("previousClose") or 180.0
                shares = (
            ...
            print(f"Warning: Failed to fetch online data for {ticker}: {e}")

    if not online_success:
        print(f"Using robust mock/sample data for {ticker}...")
        years = ["2021", "2022", "2023", "2024"]

        # Financials (AAPL-like for demonstration)
        fin_data = {
        ...
        data = {
            ...
            else f"{ticker.upper()} (Sample Data)",
            "price": 180.0,
            "shares": 15000000000,
            "beta": 1.1,
...
# ── MAIN PDF COMPILER ───────────────────────────────────────────────────
def build_pdf_report(ticker):
    # 1. Fetch data
    data = get_company_data(ticker)

    # 2. Extract Dataframes and information
    ...
    # Check actual columns (years)
    years = list(df_fin.columns)
    # Sort columns chronologically if necessary
    try:
    ...
    total_liab = find_row(df_bal, ["total liabilities"])
    ...
    ocf = find_row(
        df_cf, ["operating cash flow", "cash flow from operating activities"]
    )

    # Compute Ratios
    curr_ratios = curr_assets / curr_liab
    gross_margin = (revenue - find_row(df_fin, ["cost of revenue"])) / revenue * 100
    operating_margin = operating_income / revenue * 100
    net_margin = net_income / revenue * 100

    # Clean up goodwill-adjusted equity
    adj_assets = total_assets - goodwill

    # 3. STATS & EVALUATION FOR THE EXPRESS ALGORITHM
    sins = []
    ...
    if latest_cr < 1.0:
        sins.append(
            f"Критическая ликвидность: коэффициент текущей ликвидности (Current Ratio) ниже 1.0 ({latest_cr:.2f}), что указывает на дефицит оборотного капитала."
        )
    elif latest_cr < 1.5:
        sins.append(
            f"Сниженная ликвидность: коэффициент текущей ликвидности составляет всего {latest_cr:.2f}, желательно держать выше 1.5-2.0."
        )

    # Check Current Ratio Trend
    if len(curr_ratios) >= 2:
        if curr_ratios.iloc[-1] < curr_ratios.iloc[-2]:
            sins.append(
                f"Снижающийся тренд ликвидности: Current Ratio снизился с {curr_ratios.iloc[-2]:.2f} до {curr_ratios.iloc[-1]:.2f}."
            )
    ...
    if latest_equity <= 0:
        sins.append(
            "Отрицательный акционерный капитал: у компании больше обязательств, чем реальных активов. Сильнейший риск банкротства."
        )
    elif len(equity) >= 2 and equity.iloc[-1] < equity.iloc[-2]:
        sins.append(
            "Тренд падения капитала: акционерный капитал (Shareholder Equity) снизился за последний год."
        )

    # Rule 3: Free Cash Flow Check
    latest_fcf = fcf.iloc[-1]
    if latest_fcf <= 0:
        sins.append(
            "Сжигание денежных средств: отрицательный Свободный денежный поток (Free Cash Flow) - компания тратит больше, чем зарабатывает."
        )
    elif len(fcf) >= 2 and fcf.iloc[-1] < fcf.iloc[-2]:
        sins.append(
            "Падение денежного потока: наблюдается снижение FCF за последний год."
        )

    # Rule 4: Revenue & Profit Trend
    if len(revenue) >= 2 and revenue.iloc[-1] < revenue.iloc[-2]:
        sins.append("Снижение выручки: объём продаж за последний год сократился.")
    if len(net_income) >= 2 and net_income.iloc[-1] < net_income.iloc[-2]:
        sins.append("Падение чистой прибыли: прибыль за последний год уменьшилась.")

    # Rule 5: Margin Pressure
    if len(net_margin) >= 2 and net_margin.iloc[-1] < net_margin.iloc[-2]:
        sins.append(
            f"Падение рентабельности: чистая маржа снизилась с {net_margin.iloc[-2]:.1f}% до {net_margin.iloc[-1]:.1f}%."
        )
    ...
    if len(sins) == 0:
        ...
        reasoning = "Отличный сильный бизнес, однако в финансовых трендах или балансе присутствуют незначительные погрешности (небольшие провалы прибыли или тренд снижения ликвидности). Рекомендуется покупка только ограниченной долей."
    else:
    ...
    if len(fcf_values) >= 2 and fcf_values[0] > 0 and fcf_values[-1] > 0:
        ...
        # Bounded between 2% and 15%
        cagr = max(0.02, min(0.15, cagr))
    else:
        cagr = 0.05  # default estimate

    # Cost of Equity / Debt for WACC
    # CAPM: Rf + beta * ERP
    rf_rate = 0.04
    erp = 0.05
    ...
    chart_img_path = generate_fcf_chart(
        year_labels, fcf_values, proj_years, projected_fcfs, ticker
    )
    ...
    story.append(
        Paragraph(
            f"ФУНДАМЕНТАЛЬНЫЙ АНАЛИЗ &amp; ОЦЕНКА DCF: {ticker.upper()}", title_style
        )
    )
    story.append(
        Paragraph(
    ...
    if sins:
        ...
        story.append(
            CalloutBox(
                sins_text, USABLE_W, COLORS, callout_text_style, COLORS["danger"]
            )
        )
    else:
        story.append(
    ...
    story.append(
        Paragraph("2. Экспресс-анализ финансовых результатов и баланса", h1_style)
    )
    story.append(
        Paragraph(
    ...
    # Assemble fundamental table rows
    fund_headers = [
        "Показатель (в млн. USD)",
        year_labels[0],
        year_labels[1],
        year_labels[2],
        year_labels[3],
    ...
    fund_rows = [
        [
            "Выручка (Revenue)",
            f"{revenue.iloc[0] / 1e6:,.1f}",
            f"{revenue.iloc[1] / 1e6:,.1f}",
            f"{revenue.iloc[2] / 1e6:,.1f}",
            f"{revenue.iloc[3] / 1e6:,.1f}",
        ],
        [
            "Операционная прибыль (Operating Income)",
            f"{operating_income.iloc[0] / 1e6:,.1f}",
            f"{operating_income.iloc[1] / 1e6:,.1f}",
            f"{operating_income.iloc[2] / 1e6:,.1f}",
            f"{operating_income.iloc[3] / 1e6:,.1f}",
        ],
        [
            "Чистая прибыль (Net Income)",
            f"{net_income.iloc[0] / 1e6:,.1f}",
            f"{net_income.iloc[1] / 1e6:,.1f}",
            f"{net_income.iloc[2] / 1e6:,.1f}",
            f"{net_income.iloc[3] / 1e6:,.1f}",
        ],
        [
        ...
        [
            ...
            f"{curr_assets.iloc[0] / 1e6:,.1f}",
            f"{curr_assets.iloc[1] / 1e6:,.1f}",
            f"{curr_assets.iloc[2] / 1e6:,.1f}",
            f"{curr_assets.iloc[3] / 1e6:,.1f}",
        ],
        [
            "Краткосрочные обязательства (Current Liab)",
            f"{curr_liab.iloc[0] / 1e6:,.1f}",
            f"{curr_liab.iloc[1] / 1e6:,.1f}",
            f"{curr_liab.iloc[2] / 1e6:,.1f}",
            f"{curr_liab.iloc[3] / 1e6:,.1f}",
        ],
        [
            "Текущая ликвидность (Current Ratio)",
            f"{curr_ratios.iloc[0]:.2f}",
            f"{curr_ratios.iloc[1]:.2f}",
            f"{curr_ratios.iloc[2]:.2f}",
            f"{curr_ratios.iloc[3]:.2f}",
        ],
        [
            "Акционерный капитал (Shareholders Equity)",
            f"{equity.iloc[0] / 1e6:,.1f}",
            f"{equity.iloc[1] / 1e6:,.1f}",
            f"{equity.iloc[2] / 1e6:,.1f}",
            f"{equity.iloc[3] / 1e6:,.1f}",
        ],
        [
    ...
    story.append(
        create_reportlab_table(
            fund_headers, fund_rows, styles, COLORS, col_widths=[190, 70, 70, 70, 70]
        )
    )
    story.append(Spacer(1, 10))
    ...
    story.append(
        Paragraph("3. Модель дисконтирования денежных потоков (DCF)", h1_style)
    )
    story.append(
        Paragraph(
    ...
    dcf_info_text = (
        ...
        f"• <b>Терминальный темп роста:</b> {terminal_g * 100:.1f}% (пожизненный темп роста компании в постпрогнозный период)<br/>"
    ...
    story.append(
        CalloutBox(
            dcf_info_text, USABLE_W, COLORS, callout_text_style, COLORS["accent"]
        )
    )
    story.append(Spacer(1, 8))

    # Value Verdict and Margin of Safety
    val_banner_text = (
    ...
    proj_rows = [
        [
            ...
            f"{projected_fcfs[0] / 1e6:,.1f}",
            f"{projected_fcfs[1] / 1e6:,.1f}",
            f"{projected_fcfs[2] / 1e6:,.1f}",
            f"{projected_fcfs[3] / 1e6:,.1f}",
            f"{projected_fcfs[4] / 1e6:,.1f}",
        ],
        [
    ...
    story.append(
        create_reportlab_table(sensitivity_headers, sensitivity_rows, styles, COLORS)
    )
    story.append(Spacer(1, 12))

    # Technical analysis reminder from the lecture
    warning_text = (
    ...
    story.append(
        CalloutBox(
            warning_text, USABLE_W, COLORS, callout_text_style, COLORS["warning"]
        )
    )
