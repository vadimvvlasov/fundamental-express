...
engine in financial_analyzer.py (get_company_data/compute_metrics) and the
shared renderers (reporting/markdown.py, reporting/pdf.py,
...
implementations on top of their own compute_*_metrics() engines - neither
delegates to OrdinaryAnalyzer anymore.

T21 (docs/spec/refactor-tasks.md): every generate_pdf_report()/
generate_markdown_report() below reads self.data/self.metrics - already
populated by fetch_data()/calculate_metrics() - instead of re-fetching and
re-computing internally the way build_pdf_report() et al. still do for
their own direct CLI callers (financial_analyzer.py's __main__, which
never goes through this factory). retries/retry_delay/allow_sample are
fetch_data()-only concerns now; render() never sees them.
"""
...
class BaseAnalyzer(ABC):
    ...
    def analyze(self):
        ...
        does - it only builds the comparative report), and requires
        fetch_data()/calculate_metrics() to have already run (T21 -
        generate_pdf_report()/generate_markdown_report() no longer fetch
        or compute on their own)."""
        self.fetch_data()
...
class OrdinaryAnalyzer(BaseAnalyzer):
    ...
    def _sections(self):
        """Shared by generate_markdown_report()/generate_pdf_report() - both
        need the same ordinary sections list built from self.data/self.metrics
        (already populated - see class/module docstring)."""
    ...
    def generate_markdown_report(self):
        content = md_render(
            self.ticker, self.data, self.metrics, self._sections(),
    ...
    def generate_pdf_report(self):
        return pdf_render(
            self.ticker, self.data, self.metrics, self._sections(), OUTPUT_DIR,
            getattr(self.args, "excluded_sector", None),
            getattr(self.args, "excluded_industry", None),
        )
...
class BankAnalyzer(BaseAnalyzer):
    ...
    reporting/pdf.py::render()/reporting/markdown.py::render() (spec Section 6).
    ...
    def generate_markdown_report(self):
        content = md_render(self.ticker, self.data, self.metrics, self._sections())
        return md_write(self.ticker, content, OUTPUT_DIR)

    def generate_pdf_report(self):
        return pdf_render(self.ticker, self.data, self.metrics, self._sections(), OUTPUT_DIR)
...
class AnalyzerFactory:
    ...
    def get_analyzer(ticker, args, info):
        ...
        reporting/markdown.py::render()/reporting/pdf.py::render() stays
        exercised by its existing tests even though no live sector
        currently triggers it.

        Stashes excluded_sector/excluded_industry onto `args` so
        OrdinaryAnalyzer's generate_markdown_report()/generate_pdf_report()
        can thread them into render()'s warning-banner rendering without
        another signature change. Set to None for the bank/REIT branches,
