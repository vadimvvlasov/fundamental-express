...
- **`portfolio_analyzer.py` показывал «X/5» грехов** — число было взято из воздуха: в
  чеклисте 7 независимых проверок, не 5. Заменено на `MAX_SINS` (константа в
  `financial_analyzer.py`), чтобы «X из N» никогда не расходилось с реальным числом
  проверок в коде.
...
1. **Уровень ликвидности** — Current Ratio < 1.0 (критично) либо < 1.5 (снижена).
2. **Тренд ликвидности** — Current Ratio упал по сравнению с прошлым годом.
3. **Акционерный капитал** — Shareholders Equity ≤ 0 (отрицательный капитал) либо
   упал по сравнению с прошлым годом.
4. **Свободный денежный поток** — FCF ≤ 0 (компания жжёт кэш) либо упал по
   сравнению с прошлым годом.
5. **Выручка** — упала по сравнению с прошлым годом.
6. **Чистая прибыль** — упала по сравнению с прошлым годом.
7. **Чистая маржа** — упала по сравнению с прошлым годом.

Максимум **7** сработавших пунктов (пары «уровень/тренд» — 1 и 2, 3, 4 — взаимоисключающие
внутри себя, поэтому не 5, как могло бы показаться, а именно 7). Вердикт:
**0** грехов — 🟢 купить, **1–2** — 🟡 наблюдать, **3+** — 🔴 пропустить.
